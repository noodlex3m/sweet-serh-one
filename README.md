...
1. **🤖 Telegram-бот для автоматичних сповіщень (Serverless):**
   ...
   * *Концепція налаштування:* Створюється приватна Telegram-група/канал складу, куди додається створений бот. ID чату та токен бота зберігаються у змінних оточення Netlify (див. інструкцію нижче).
...
4. **🏦 Виставлення рахунків на IBAN ФОП (PDF-рахунок):**
   * Автоматичне виставлення рахунків на оплату за реквізитами ФОП (налаштовуються через змінні середовища, без зашивання персональних даних у Git).
   * Автоматична генерація простого PDF-документа (Рахунок-фактура + Накладна) для скачування покупцем або друку.
5. **🛍️ Гнучке керування цінами (Опт / Роздріб):**
   * Налаштування гуртової ціни (при купівлі від 1 ящика / 5 кг) та роздрібної для кожної картки товару.
6. **📝 Редагування складу замовлень адміністратором:**
   * Можливість для адміністратора змінювати кількість товарів, додавати або видаляти позиції в замовленні на етапі перевірки замовлення на складі (при статусах «Нове» та «Очікує оплати»).
